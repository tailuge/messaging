...
describe("Lobby - Sentinel Failure", () => {
  it("should keep users in unsettled state when sentinel never arrives", async () => {
    ...
    // Type assertion for accessing private properties in test
    const lobbyExtended = lobby as any;
    ...
    console.log("Users in unsettled buffer:", lobbyExtended.unsettledPresenceMessages.length);
    ...
    expect(lobbyExtended.unsettledPresenceMessages.length).toBeGreaterThan(0); // Messages in buffer
    
    // Additional diagnostic: Check that the sentinel was NOT received
    // (if it had been received, lobby.settled would be true)
    expect(lobbyExtended.joinSentinelTs).toBe(expectedSentinelTs);
  });
});
