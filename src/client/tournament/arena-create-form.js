...
class ArenaCreateForm extends LitElement {
    ...
    static styles = css`
        ...
        .config { display: flex; align-items: center; justify-content: flex-end; gap: .3rem; padding: .45rem; border: 1px dashed var(--border); border-radius: 4px; }
    ...
    render() {
        return html`<div class="field"><label>Game parameters</label><div class="config"><div class="config-actions"><button type="button" class="btn-preset" title="10 mins Three Cushion (mini, race to 7)" @click=${() => this._selectPreset('threecushion', { raceTo: '7', tableSize: '5' }, 10)}><img src="assets/threecushion.png" alt="" /><span>3-Cushion</span></button><button type="button" class="btn-preset" title="10 mins Nine Ball (mini, freeaim)" @click=${() => this._selectPreset('nineball', { tableSize: '6', freeaim: 'true' }, 10)}><img src="assets/nineball.png" alt="" /><span>9-Ball</span></button><button type="button" class="btn-preset" title="10 mins Eight Ball (mini, freeaim)" @click=${() => this._selectPreset('eightball', { tableSize: '6', freeaim: 'true' }, 10)}><img src="assets/eightball.png" alt="" /><span>8-Ball</span></button><button type="button" @click=${this._openChooser}>Choose</button></div></div></div><div class="field"><label for="duration">Duration</label><select id="duration" .value=${String(this.durationMinutes)} @change=${this._onDurationChange}><option value="10">10 minutes</option><option value="30">30 minutes</option></select></div>${this.error ? html`<div class="error" role="alert">${this.error}</div>` : ''}<button class="btn-challenge create" type="button" ?disabled=${this.busy} @click=${this._create}>${this.busy ? 'Creating…' : 'Create Arena'}</button><proto2-modal @confirm=${this._onParameters}></proto2-modal>`;
    }
}
