...
class OnlinePanel extends LitElement {
    ...
    async #challenge(userId, ruleType, options) {
        ...
        if (u?.isBot) {
            ...
            window.location.href = gameUrl({ tableId, userId: this.#myId, userName: this.#myName, ruleType, isFirst, options, bot: u.userName, lod: userStore.lod, flipX: userStore.flipX, rematch: this.#rematch?.rematchParam });
            return;
        }
    ...
    render() {
        if (this.#tableId) {
            const url = gameUrl({ tableId: this.#tableId, userId: this.#myId, userName: this.#myName, ruleType: this.#ruleType, isFirst: this.#isFirst, options: this.#matchOptions, lod: userStore.lod, flipX: userStore.flipX, rematch: this.#state.currentMatch?.rematch });
